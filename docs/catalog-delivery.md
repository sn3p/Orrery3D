...
The tracked selection uses the published
[`latest.json`](https://sn3p.github.io/orrery-data/latest.json) endpoint. Normal
build, serve and watch commands need no local dataset, cache or data-host access.
The browser discovers the current index and loads verified chunks directly from
orrery-data Pages. Missing data shows a loading error; there is no historical fallback.
...
The application uses **one producer-owned data host**. The producer updates the
current generated browser chunks and index in `orrery-data` only when their
content changes, and serves them from that repository's static site.
This app discovers the current dataset and requests chunks directly at
...
The producer shipped this smaller browser distribution in [PR5](https://github.com/sn3p/orrery-data/pull/5) and published it through Pages. This app now
...
Further producer changes belong in a fresh `orrery-data` review unit. This preference does
...
For optional complete-bundle comparisons, save this configuration outside Git, for example as
...
Do not describe two bundles as unlimited stale-client compatibility. The normal shared browser distribution is hosted by the producer and is not included in this artifact-size calculation.
[GitHub Pages limits](https://docs.github.com/en/pages/getting-started-with-github-pages/github-pages-limits).

## Verification and remaining rollout limits

The normal entry uses the published producer distribution. A clean app build
needs only source and npm dependencies. The default-command regression forbids
catalogue fetches and local-cache reads during the actual npm build and checks
that the artifact contains the hosted URL without a dataset copy.

The producer owns data refresh and publication. A consumer reload revalidates
`latest.json`; an existing session keeps its selected index. If a refreshed
publication removes its old chunks, loading stops with a reload message.
Indefinite old-version availability is not required. The fixture workflows test
replacement, corruption and reload without changing public data.

After this PR merges, the app's Pages workflow publishes the same tested default.
Deployment at the public `/Orrery3D/` URL still needs a post-deployment smoke check.
Physical-phone performance and maximum-speed buffering/late-start costs remain
explicit limits from the [rollout handoff](catalog-trial.md#before-public-rollout).
Desktop and narrow-browser checks do not establish performance on actual phones.
